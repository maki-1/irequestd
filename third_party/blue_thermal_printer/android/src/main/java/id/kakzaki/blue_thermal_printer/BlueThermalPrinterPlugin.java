...
public class BlueThermalPrinterPlugin implements FlutterPlugin, ActivityAware,MethodCallHandler, RequestPermissionsResultListener {
  ...
  /**
   * Cheap thermal printers (this plugin was built against the Xprinter XP-58H)
   * frequently have an incomplete SDP record, so the standard
   * createRfcommSocketToServiceRecord(SPP UUID) handshake opens a socket that
   * then dies immediately on the first read ("read failed, socket might
   * closed or timeout") — a long-documented issue for exactly this class of
   * device. The widely-used workaround is a second attempt on a raw,
   * insecure RFCOMM channel (channel 1, which is where SPP almost always
   * lives on these printers), reached via reflection since
   * BluetoothDevice.createRfcommSocket(int) isn't public API.
   */
  private BluetoothSocket connectWithFallback(BluetoothDevice device) throws Exception {
    try {
      ...
      Log.w(TAG, "standard SPP connect failed (" + primaryFailure.getMessage()
          + "), retrying on insecure RFCOMM channel 1");
      try {
        ...
        // Neither path worked — surface the original error, it's the more
        // informative one (the fallback's is almost always a plain
        // reflection wrapper exception).
